...
export function createMdProcessor(ctx: BuildCtx): QuartzMdProcessor {
  ...
  return (
    unified()
      ...
      .use(remarkParse)
      // Recognise the leading `---...---` YAML block as a frontmatter node
      // (otherwise it leaks into the body as a setext heading). remark-rehype
      // drops `yaml` nodes, so this also strips it from rendered output.
      .use(remarkFrontmatter, ["yaml"])
      // Parse that YAML into file.data.frontmatter so titles/aliases/tags/unlisted
      // are available to the explorer, graph, article-title, and other plugins.
      .use(() => (tree, file) => {
        visit(tree, "yaml", (node: { value: string }) => {
          let data: Record<string, unknown> = {}
          try {
            data = (parseYaml(node.value, { uniqueKeys: false }) as Record<string, unknown>) ?? {}
          } catch {
            data = {}
          }
          if (typeof data.aliases === "string") data.aliases = [data.aliases]
          if (typeof data.tags === "string") data.tags = [data.tags]
          file.data.frontmatter = { title: "Untitled", ...data } as typeof file.data.frontmatter
        })
      })
