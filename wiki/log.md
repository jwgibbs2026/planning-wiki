...
## [2026-06-15] build | Planning Application Mapping Dashboard (Phase 1)

Built a private, local mapping dashboard as a self-contained Python subproject under `dashboard/` (separate venv/stack from the markdown vault), answering the long-standing purpose.md question *"What planning applications are happening near me?"*

**Delivered (Phase 1, working & verified):** PlanIt API importer → SQLite (SpatiaLite optional, GeoJSON-text fallback) → FastAPI GeoJSON API → Leaflet map. Monitors all 7 Gloucestershire authorities (Gloucester City, Cheltenham, Gloucestershire County, Tewkesbury, Forest of Dean, Stroud, Cotswold); each application links to its official council PublicAccess page. Incremental daily import via PlanIt `changed_start` with per-authority fault isolation. Verified end-to-end: 1,928 applications imported, map renders clustered points with working popups/filters, 6 passing unit tests.

**Scaffolded (Phases 2–4, stubs only):** modular per-council document scrapers (`scrapers/`), Claude VLM drawing classification (`classify/`), and OpenCV red-line boundary extraction (`extract/`) — interfaces and DB tables in place, logic deferred.

**Wiki changes:** created [[Planning Application Mapping Dashboard]] (`wiki/dashboard.md`); added a "Tools" section to `wiki/index.md`.
